...
class ProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_url_fetch(self):
        ...
        try:
            resp = get_url_service.get_url(url,
                                           method=method,
                                           headers=headers,
                                           data=body,
...
class FuckAsyncProxyServer(HttpProxyServer):
    ...
    async def join(self, timeout=None):
        return await asyncio_helper.get_running_loop().run_in_executor(None,super().join)
...
__all__ = ["HttpProxyServer","FuckAsyncProxyServer"]