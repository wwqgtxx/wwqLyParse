...
class _SSLProtocolTransport(transports._FlowControlMixin,
    ...
    def __del__(self):
        if not self._closed:
            warnings.warn("unclosed transport {!r}".format(self), ResourceWarning)
            self.close()
    ...
    def write(self, data):
        ...
        if not isinstance(data, (bytes, bytearray, memoryview)):
            raise TypeError("data: expecting a bytes-like instance, got {}".format(type(data).__name__))
        if not data:
            return
...
class SSLProtocol(protocols.Protocol):
    ...
    def __init__(self, loop, app_protocol, sslcontext, waiter,
        ...
        if ssl_handshake_timeout is None:
            ...
            raise ValueError(
                "ssl_handshake_timeout should be a positive number, got {}".format(ssl_handshake_timeout))
    ...
    def _check_handshake_timeout(self):
        if self._in_handshake is True:
            msg = (
                "SSL handshake is taking longer than {} seconds: aborting the connection".format(self._ssl_handshake_timeout)
            )
