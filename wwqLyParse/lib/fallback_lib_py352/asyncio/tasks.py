            ...
            if blocking is not None:
                ...
                if futures._get_loop(result) is not self._loop:
                    new_exc = RuntimeError(
                        'Task {!r} got Future {!r} attached to a different loop'.format(self, result))
                    ...
                    new_exc = RuntimeError(
                        'yield was used instead of yield from in task {!r} with {!r}'.format(self, result))
                    self._loop.call_soon(
                        self.__step, new_exc, context=self._context)
                ...
                new_exc = RuntimeError(
                    'yield was used instead of yield from for generator in task {!r} with {!r}'.format(self, result))
                self._loop.call_soon(
                    self.__step, new_exc, context=self._context)
            else:
                # Yielding something else is an error.
                new_exc = RuntimeError('Task got bad yield: {!r}'.format(result))
                self._loop.call_soon(
                    self.__step, new_exc, context=self._context)
        finally:
...
async def wait(fs, *, loop=None, timeout=None, return_when=ALL_COMPLETED):
    ...
    if futures.isfuture(fs) or coroutines.iscoroutine(fs):
        raise TypeError("expect a list of futures, not {}".format(type(fs).__name__))
    if not fs:
...
def _enter_task(loop, task):
    ...
    if current_task is not None:
        raise RuntimeError(
            "Cannot enter into task {!r} while another task {!r} is being executed.".format(task, current_task))
...
def _leave_task(loop, task):
    ...
    if current_task is not task:
        raise RuntimeError("Leaving task {!r} does not match the current task {!r}.".format(task, current_task))
